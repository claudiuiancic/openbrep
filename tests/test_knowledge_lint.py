...
class TestKnowledgeLint(unittest.TestCase):
    ...
    def test_officially_verified_wiki_pages_do_not_reintroduce_pseudo_syntax(self):
        ...
        group = (root / "GROUP.md").read_text(encoding="utf-8")
        ...
        self.assertIn('GROUP "name"', group)
        self.assertIn("PLACEGROUP g_expr", group)
        self.assertIn("KILLGROUP g_expr", group)
        self.assertIn("Group definitions are not visible geometry by themselves", group)
        self.assertIn("Group definitions cannot be nested", group)
        self.assertNotIn('GROUP "name" [, pen, material]', group)
        ...
        self.assertIn("Advanced non-default command", revolve)
        self.assertNotIn("```gdl\nREVOLVE id, angle", revolve)
        self.assertIn("SWEEP n, m, alpha, scale, mask", sweep)
        self.assertIn("Advanced non-default command", sweep)
